"""
S2_threshold_sensitivity.py  —  Threshold-sensitivity analysis
Recomputes regime labels under alternative threshold schemes and repeats the
rolling out-of-sample comparison for Persistence, HAR-RV, MS-HAR, Hard-HMM,
and RAFT. Used to verify that the main conclusions are not driven by the
baseline contagion-label calibration.
...
_CFG = {}
SCHEMES = {
    'Loose (92.5/85/75)':    {'rv_max': 92.5, 'corr': 85.0, 'asset': 75.0},
    'Strict (97.5/92.5/85)': {'rv_max': 97.5, 'corr': 92.5, 'asset': 85.0},
}
...
def load_data(cfg: dict) -> pd.DataFrame:
    ...
    df = df.dropna(subset=['rv_lag1', 'rv_lag24', 'rv_lag168', 'cs_hvol_std_l1', 'cs_hvol_max_l1', 'extreme_move_l1']).reset_index(drop=True)
...
def compute_thresh_quantiles(df_tr: pd.DataFrame, scheme: dict) -> dict:
    return {
        ...
        'per_asset_q': {s: np.nanpercentile(df_tr[f'{s}_hvol_l1'].values, scheme['asset']) for s in SHORTS if f'{s}_hvol_l1' in df_tr.columns},
    }

def threshold_regime(df: pd.DataFrame, thr: dict) -> np.ndarray:
    per_q   = thr['per_asset_q']
    n_above = sum((df[f'{s}_hvol_l1'].values > per_q[s]).astype(int) for s in SHORTS if f'{s}_hvol_l1' in df.columns and s in per_q)
    contagion_mask = (df['cs_hvol_max_l1'].values > thr['tw_q_rv_max']) & (df['cs_hvol_std_l1'].values > thr['tw_q_cs_std']) & (n_above >= 3)
...
def _sort_by_label_alignment(beta, sigma2, Pi, gamma, k_label, K):
    ...
    best_perm = max(permutations(range(K)), key=lambda p: sum(score_mat[i, p[i]] for i in range(K)))
    bp = list(best_perm)
...
def _concat_preds(preds, regime_filter=None):
    ...
    return np.concatenate(mus), np.concatenate(ys), np.concatenate(s2s)
...
def _run_one_window(args):
    ...
    regime_te   = df_te['scheme_regime'].fillna('Normal').values.astype(str)
    ...
    try:
        ...
        out['Persistence'] = {
            'mu':     df_te['rv_lag1'].values,
            'sigma2': np.full(len(df_te), s2_pers),
    ...
    try:
        ...
        out['HAR-RV'] = {
            'mu':     _har_X(df_te) @ beta_har,
            'sigma2': np.full(len(df_te), s2_har),
            'y': df_te['log_rv'].values, 'regime': regime_te,
        }
    except Exception as e:
        print(f'  HAR-RV @ {ws}: {e}')
    ...
    try:
        ...
        X_te  = _har_X(df_te)
        out['MS-HAR'] = {
            'mu':     np.array([X_te[t] @ beta[k_te_thresh[t]] for t in range(len(df_te))]),
            'sigma2': np.array([sigma2[k_te_thresh[t]] for t in range(len(df_te))]),
            'y': df_te['log_rv'].values, 'regime': regime_te,
        }
    except Exception as e:
        print(f'  MS-HAR @ {ws}: {e}')
    ...
    if raft_fit is not None:
        try:
            mu_hard, s2_hard = _predict_hard_hmm(df_te, raft_fit['beta'], raft_fit['sigma2'], raft_fit['Pi'])
            out['Hard-HMM'] = {
                'mu': mu_hard, 'sigma2': s2_hard,
                'y': df_te['log_rv'].values, 'regime': regime_te,
            }
        except Exception as e:
...
def run_scheme(df_base: pd.DataFrame, cfg: dict, scheme_name: str, scheme: dict) -> pd.DataFrame:
    calib_h = cfg['analysis']['regime_calib_hours']
    df      = build_scheme_labels(df_base, scheme, calib_h)
    valid   = df.iloc[720:]
    cont_share = 100.0 * (valid['scheme_regime'] == 'Contagion').mean()
    fc       = cfg['forecast']
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as pool:
        futs = {pool.submit(_run_one_window, (ws, df_tr, df_te, scheme, dict(cfg))): ws
    ...
    for model_name in MODEL_ORDER:
        ...
        if not preds:
            continue
        row = {'Scheme': scheme_name, 'Contagion Share': round(cont_share, 1), 'Model': model_name}
        for label, rf in groups.items():
            mu, y, s2 = _concat_preds(preds, rf)
            if mu is None or len(mu) < 5:
                row[f'{label}_QLIKE'] = np.nan
                row[f'{label}_LogS']  = np.nan
            else:
                row[f'{label}_QLIKE'] = round(qlike(mu, y), 3)
                row[f'{label}_LogS']  = round(log_score(y, mu, s2), 3)
        rows.append(row)
    return pd.DataFrame(rows)

def print_table(table: pd.DataFrame) -> None:
    hdr = (f"{'Model':<12} {'All QLIKE':>10} {'All Log-S':>10} "
           f"{'Cont QLIKE':>11} {'Cont Log-S':>11} "
           f"{'Stress QLIKE':>13} {'Stress Log-S':>13} "
           f"{'Norm QLIKE':>11} {'Norm Log-S':>11}")
    sep = '=' * 114
    print(f'\n{sep}')
    print('TABLE S2: Supplementary Threshold Sensitivity Results')
    print(sep)
    for scheme_name in list(SCHEMES.keys()):
        sub   = table[table['Scheme'] == scheme_name]
        if sub.empty:
            continue
        share = float(sub['Contagion Share'].iloc[0])
        print(f'\nScheme: {scheme_name} | Contagion share: {share:.1f}%')
        print('-' * 114)
        print(hdr)
...
def parse_args():
    p = argparse.ArgumentParser(description='Threshold sensitivity QLIKE/LogS')
...
def main():
    ...
    for scheme_name, scheme in SCHEMES.items():
        ...
        tables.append(run_scheme(df, cfg, scheme_name, scheme))
    table = pd.concat(tables, ignore_index=True)
    table['Scheme'] = pd.Categorical(table['Scheme'], categories=list(SCHEMES.keys()), ordered=True)
    table['Model']  = pd.Categorical(table['Model'],  categories=MODEL_ORDER, ordered=True)
    table = table.sort_values(['Scheme', 'Model']).reset_index(drop=True)
    out = cfg['_results_dir'] / 'table_s2_threshold_sensitivity.csv'
    table.to_csv(out, index=False)
    print(f'\nSaved {out}')
    print('\nContagion share by scheme (baseline: 4.4%):')
    for scheme_name in list(SCHEMES.keys()):
        share = float(table[table['Scheme'] == scheme_name]['Contagion Share'].iloc[0])
        print(f'  {scheme_name}: {share:.1f}%')
    print_table(table)
